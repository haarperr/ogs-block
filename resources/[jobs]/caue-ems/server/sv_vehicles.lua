...
AddEventHandler("caue-ems:purchaseVehicle", function(params)
    ...
    local vid = exports["caue-vehicles"]:insertVehicle(src, params["model"], "ems", params["price"], false, true)
    if not vid then
        TriggerClientEvent("DoLongHudText", src, "Error??", 2)
        return
    end

    exports["caue-vehicles"]:updateVehicle(vid, "garage", "garage", "Hospital")
